...
def get_parser():
    ...
    parser.add_argument(
        "--json-input", help="input json file for detection in batch mode"
    )
    parser.add_argument(
        "--output",
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        with open(args.json_input) as f:
            inputs = json.load(f)
        for path in tqdm.tqdm(inputs["images"], disable=not args.output):
            # use PIL, to be consistent with evaluation
            img = read_image(
                f"{'/'.join(args.json_input.split('/')[0:2])}/{path['file_name']}",
                format="BGR",
            )
            start_time = time.time()
            predictions, visualized_output = demo.run_on_image(img)
            ...
            if args.output:
                if os.path.isdir(args.output):
                    ...
                    out_filename = os.path.join(
                        args.output, os.path.basename(path["file_name"])
                    )
                else:
