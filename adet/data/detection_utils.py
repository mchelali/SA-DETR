...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if "beziers" in annos[0]:
        beziers = np.array([obj.get("beziers", []) for obj in annos])
    ...
    if "text" in annos[0]:
        ...
        instance.texts = torch.as_tensor(texts, dtype=torch.long)

    if "language" in annos[0]:
        languages = [obj.get("language", []) for obj in annos]
        instance.languages = torch.as_tensor(languages, dtype=torch.long)

    return instance


def language_name2idx(lan_name):
    idx_dict = {
        "Arabic": 0,
        "Bangla": 1,
        "Chinese": 2,
        ...
        "Latin": 6,
        "Symbols": 7,
    }  # TODO: check here for different language settings
    return idx_dict[lan_name]
