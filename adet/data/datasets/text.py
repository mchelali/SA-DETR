...
from adet.data.detection_utils import language_name2idx
...
def register_text_instances(name, metadata, json_file, image_root, num_pts_cfg):
    """
    ...
    DatasetCatalog.register(
        name,
        lambda: load_text_json(json_file, image_root, name, num_pts_cfg=num_pts_cfg),
    )
...
def register_stamp_instances(name, metadata, json_file, image_root, num_pts_cfg):
    """
    Register a dataset in json annotation format for text detection and recognition.
...
def load_text_json(
    json_file, image_root, dataset_name=None, extra_annotation_keys=None, num_pts_cfg=25
):
    """
    ...
    for img_dict, anno_dict_list in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            if text:
                ...
                text_check = np.sum(text_check != 0)
                # filter the instance without text script
                if text_check == 0:
                    continue
                obj["text"] = text
            else:
                obj["text"] = np.zeros(len(obj["polyline"]), dtype=np.int32)

            language = anno.get("language", None)
            if language:
                obj["language"] = language_name2idx(language)
