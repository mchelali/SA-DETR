...
def detector_postprocess(
    results, output_height, output_width, min_size=None, max_size=None
):
    """
    scale align
    """
...
class TransformerPureDetector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        backbone = Joiner(
            ...
            PositionalEncoding2D(
                N_steps, cfg.MODEL.TRANSFORMER.TEMPERATURE, normalize=True
            ),
        )
        ...
        weight_dict = {
            ...
            "loss_languages": loss_cfg.LANGUAGE_WEIGHT,
        }
        ...
        enc_losses = ["labels_enc", "beziers"]
        dec_losses = ["labels_dec", "ctrl_points", "bd_points", "languages", "texts"]
        ...
        self.criterion = SetCriterion(
            ...
            dec_losses,
            cfg.MODEL.TRANSFORMER.LOSS.TEXT_PENALTY,
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            text = output["pred_text_logits"]
            bd_points = output["pred_bd_points"]
            languages = output["pred_lan_logits"]
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            # handle images with no instances or missing fields
            h, w = targets_per_image.image_size

            gt_classes = getattr(targets_per_image, "gt_classes", None)
            ...
            raw_beziers = getattr(
                targets_per_image,
                "beziers",
                torch.zeros((num_gt, 4, 2), dtype=torch.float32, device=self.device),
            )
            raw_ctrl_points = getattr(
                targets_per_image,
                "polyline",
                torch.zeros(
                    (num_gt, self.detection_transformer.num_points, 2),
                    dtype=torch.float32,
                    device=self.device,
                ),
            )
            raw_boundary = getattr(
                targets_per_image,
                "boundary",
                torch.zeros(
                    (num_gt, self.detection_transformer.num_points, 4),
                    dtype=torch.float32,
                    device=self.device,
                ),
            )

            gt_texts = getattr(targets_per_image, "texts", None)
            if gt_texts is None:
                gt_texts = torch.zeros(
                    (num_gt, 1), dtype=torch.long, device=self.device
                )
            else:
                gt_texts = gt_texts.to(self.device)

            gt_beziers = (
                raw_beziers.reshape(-1, 4, 2)
                / torch.as_tensor([w, h], dtype=torch.float, device=self.device)[
                    None, None, :
                ]
                if raw_beziers.numel() != 0
                else raw_beziers
            )
            gt_ctrl_points = (
                raw_ctrl_points.reshape(-1, self.detection_transformer.num_points, 2)
                / torch.as_tensor([w, h], dtype=torch.float, device=self.device)[
                    None, None, :
                ]
                if raw_ctrl_points.numel() != 0
                else raw_ctrl_points
            )
            gt_boundary = (
                raw_boundary.reshape(-1, self.detection_transformer.num_points, 4)
                / torch.as_tensor([w, h, w, h], dtype=torch.float, device=self.device)[
                    None, None, :
                ]
                if raw_boundary.numel() != 0
                else raw_boundary
            )
            ...
            languages = getattr(
                targets_per_image,
                "languages",
                torch.zeros((num_gt,), dtype=torch.long, device=self.device),
            )
    ...
    def inference(
        self, ctrl_point_cls, ctrl_point_coord, text, bd_points, languages, image_sizes
    ):
        assert len(ctrl_point_cls) == len(image_sizes)
        results = []
        prob = ctrl_point_cls.mean(-2).sigmoid()
        scores, labels = prob.max(-1)
        languages = languages.softmax(dim=-1).argmax(dim=-1)
        ...
        for idx, (
            ...
            zip(scores, labels, ctrl_point_coord, bd_points, languages, image_sizes)
        ):
            result = Instances(image_size)
            selector = scores_img >= self.test_score_threshold
            selected_idx = torch.nonzero(selector)
            scores_img = scores_img[selector]
            result.scores = scores_img
            result.pred_classes = label_img[selector]

            ctrl_point_img = ctrl_point_img[selector]
            ctrl_point_img[..., 0] *= image_size[1]
            ctrl_point_img[..., 1] *= image_size[0]
            result.ctrl_points = ctrl_point_img.flatten(1)

            bd_img = bd_img[selector]
            bd_img[..., 0::2] *= image_size[1]
            bd_img[..., 1::2] *= image_size[0]
            result.bd = bd_img

            lan_img = lan_img[selector]
            result.languages = lan_img

            text_img = [
                text[lan_idx][idx][q_idx].softmax(dim=-1).argmax(dim=-1)
                for lan_idx, q_idx in zip(lan_img.reshape(-1, 1), selected_idx)
            ]
            if text_img != []:
                text_img = torch.cat(text_img, dim=0)
            result.recs = text_img
