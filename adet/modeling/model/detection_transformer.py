...
class DETECTION_TRANSFORMER(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        self.strides = cfg.MODEL.TRANSFORMER.STRIDES
        ...
        self.point_embed = nn.Embedding(
            self.num_proposals * self.num_points, self.d_model
        )
        ...
        self.bezier_proposal_class = nn.Linear(
        ...
        self.boundary_offset = MLP(
            self.d_model, self.d_model, 4, 3
        )  # to rebuild the text boundary from queries
        self.ctrl_point_class = nn.Linear(
            self.d_model, self.num_classes
        )  # text or non-text
        self.inst_language = nn.Linear(
            self.d_model, self.language_classes
        )  # classify the language type of each instance
    ...
    def forward(self, samples: NestedTensor):
        ...
        language_token = self.language_token.weight.reshape(
            self.num_proposals, 1, self.d_model
        )
        # (n_inst x n_pts, d_model) -> (n_inst, n_pts, d_model)
        point_embed = self.point_embed.weight.reshape(
            (self.num_proposals, self.num_points, self.d_model)
        )
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_language = self.inst_language[lvl](language_hs[lvl]).squeeze(
                2
            )  # [bs, 100, lang_cls]
            outputs_text = [
                self.ctrl_point_text[lvl][lan_id](hs[lvl])
