...
def sigmoid_focal_loss(
    inputs, targets, num_inst, alpha: float = 0.25, gamma: float = 2
):
    """
...
class SetCriterion(nn.Module):
    ...
    def loss_beziers(self, outputs, targets, indices, num_inst):
        ...
        target_beziers = torch.cat(
            [t["beziers"][i] for t, (_, i) in zip(targets, indices)], dim=0
        )
    ...
    def loss_texts(self, outputs, targets, indices, num_inst):
        ...
        target_languages = torch.cat(
            [t["languages"][i] for t, (_, i) in zip(targets, indices)]
        )
        ...
        for i, lan in enumerate(language_set):
            ...
            text_head_idx_temp, target_languages_temp = (
                text_head_idx[position_idx],
                target_languages[position_idx],
            )
            target_texts_temp, target_lengths_temp = (
                target_texts[position_idx],
                target_lengths[position_idx],
            )
            target_texts_temp = torch.cat(
                [
                    t[: target_lengths_temp[t_idx]]
                    for t_idx, t in enumerate(target_texts_temp)
                ]
            )
            device = (
                position_idx.device if torch.is_tensor(position_idx) else idx[0].device
            )
            idx_temp = (
                idx[0].to(device)[position_idx],
                idx[1].to(device)[position_idx],
            )
            ...
            input_len = torch.full(
                (out_texts_temp.size(1),),
                out_texts_temp.size(0),
                dtype=torch.long,
                device=out_texts_temp.device,
            )
    ...
    def loss_ctrl_points(self, outputs, targets, indices, num_inst):
        ...
        target_ctrl_points = torch.cat(
            [t["ctrl_points"][i] for t, (_, i) in zip(targets, indices)], dim=0
        )
        loss_ctrl_points = F.l1_loss(
            src_ctrl_points, target_ctrl_points, reduction="sum"
        )
    ...
    def loss_bd_points(self, outputs, targets, indices, num_inst):
        ...
        target_bd_points = torch.cat(
            [t["bd_points"][i] for t, (_, i) in zip(targets, indices)], dim=0
        )
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.dec_losses:
            kwargs = {}
            losses.update(
                self.get_loss(loss, outputs, targets, indices, num_inst, **kwargs)
            )
