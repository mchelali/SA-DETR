...
import copy
import math
from .scale_embedder import ScaleEmbedder, ScaleEmbedder_sin, ScaleEmbedder_table
import torch
...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        self.max_log_stride = math.log2(max(strides))
        self.min_log_stride = math.log2(min(strides))
        ...
        self.decoder = DeformableCompositeTransformerDecoder(
            temp, decoder_layer, num_decoder_layers, return_intermediate_dec, d_model
        )

        # ==================== Contribution ============

        # We remove "self.level_embed"
        # self.level_embed = nn.Parameter(torch.Tensor(num_feature_levels, d_model))

        # We replace it by The ScaleEmbedder

        self.scale_embedder = ScaleEmbedder(
            d_model=d_model, K=8, sinus_dim=64, use_level_bias=False
        )

        # self.scale_embedder = ScaleEmbedder_sin(
        #     d_model=d_model, K=8, sinus_dim=64, use_level_bias=False
        # )

        # self.scale_embedder = ScaleEmbedder_table(
        #     d_model=d_model, K=8, sinus_dim=64, use_level_bias=False
        # )
        ...
        self.bernstein_matrix = torch.tensor(
            BezierCoeff(curve_token), requires_grad=False
        )
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            # --- Contribution: get scale embedding ---
            s = (math.log2(self.strides[lvl]) - self.min_log_stride) / (
                self.max_log_stride - self.min_log_stride
            )
            L = src.shape[1]
            scale_embed = self.scale_embedder(
                s, bs, L, device=src.device, level_idx=lvl
            )

            lvl_pos_embed = (
                pos_embed + scale_embed
        ...
        enc_outputs_coord_unact = (
            self.bezier_coord_embed(output_memory) + output_proposals
        )
        ...
        topk_coords_unact = torch.gather(
            enc_outputs_coord_unact, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 8)
        )
        topk_coords_unact = topk_coords_unact.detach()
        reference_points = topk_coords_unact.sigmoid()  # bs, nq, 8
        reference_points = self.init_points_from_bezier_proposals(
            reference_points
        )  # bs, nq, num_points, 2
        init_reference_out = reference_points
        # TODO: make reference points for language tokens
        #  v1: use the center point

        language_reference_points = reference_points[
            :, :, self.num_points // 2, :
        ].unsqueeze(dim=2)
        reference_points = torch.cat(
            [reference_points, language_reference_points], dim=2
        )
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        attn_mask = torch.zeros(
            (tgt.shape[2], tgt.shape[2]), dtype=torch.bool, device=tgt.device
        )
        attn_mask[:, -1] = True
        tgt2 = (
            self.attn_intra(
        ...
        q = k = tgt = torch.swapdims(tgt, 1, 2)
        tgt2 = (
            self.attn_inter(
                q.flatten(0, 1).transpose(0, 1),
                k.flatten(0, 1).transpose(0, 1),
        ...
        tgt = tgt + self.dropout_inter(tgt2)
        tgt = torch.swapdims(self.norm_inter(tgt), 1, 2)
        ...
        if len(reference_points.shape) == 4:
            ...
            assert reference_points.shape[2] == tgt.shape[2]
            reference_points_loc = reference_points

        tgt2 = self.attn_cross(
            self.with_pos_embed(tgt, query_pos).flatten(1, 2),
        ...
        tgt = tgt + self.dropout_cross(tgt2)
...
class DeformableCompositeTransformerDecoder(nn.Module):
    def __init__(
        self, temp, decoder_layer, num_layers, return_intermediate=False, d_model=256
    ):
        super().__init__()
    ...
    def forward(
        ...
        output = tgt  # [bs, 100, n_pts+1, d_model]
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                ...
                # reference_points: (bs, nq, n_pts+1, 2), reference_points_input: (bs, nq, n_pts+1, 4, 2)
                reference_points_input = (
                    reference_points[:, :, :, None] * src_valid_ratios[:, None, None]
                )

            query_pos = gen_point_pos_embed(
                reference_points_input[:, :, :, 0, :], self.d_model, self.temp
            )
            query_pos = self.ref_point_head(query_pos)  # [bs, 100, n_pts+1, d_model]
            ...
            if self.ctrl_point_coord is not None:
                tmp = self.ctrl_point_coord[lid](output[:, :, :-1, :])
            ...
            if self.return_intermediate:
                ...
                intermediate_reference_points.append(reference_points[:, :, :-1, :])

        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points)

        return output, reference_points[:, :, :-1, :]
